...
export default function PatchRatings() {
  ...
  const tableColumns = useMemo(() => [
    columnHelper.accessor('version', {
      ...
      cell: info => <span className="font-display text-xl font-black text-theme-text">v{info.getValue()}</span>
    }),
    ...
    columnHelper.display({
      ...
      cell: info => (
        <motion.button 
          ...
          className="flex items-center gap-2 bg-black text-white px-4 py-2 border-2 border-theme-border font-label text-xs font-black uppercase tracking-widest hover:bg-[var(--color-primary)] hover:text-black transition-colors w-fit"
        >
      ...
      <div className="space-y-4">
        ...
        <div className="bg-theme-bg border-4 border-theme-border shadow-[4px_4px_0px_0px_var(--color-theme-shadow)] overflow-x-auto">
              ...
              {initialFetch ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <TableRow key={i} className="border-b-4 border-theme-border last:border-b-0">
                    <TableCell className="p-6 border-r-4 border-theme-border"><Skeleton className="h-6 w-24 bg-gray-200 rounded-none" /></TableCell>
                    <TableCell className="p-6 border-r-4 border-theme-border"><Skeleton className="h-4 w-32 bg-gray-200 rounded-none" /></TableCell>
          ...
          {!initialFetch && table.getPageCount() > 1 && (
            <div className="flex items-center justify-between px-6 py-4 border-t-4 border-theme-border bg-[#f4f4f4]">
              <div className="font-label text-xs font-black text-theme-text uppercase tracking-widest bg-theme-bg border-2 border-theme-border px-3 py-1">
              ...
              <div className="flex gap-4">
                <motion.button
                  ...
                  className="px-4 py-2 bg-theme-bg border-4 border-theme-border font-display text-sm font-black uppercase disabled:opacity-50 disabled:cursor-not-allowed hover:bg-black hover:text-white transition-colors"
                >
                  Recall
                </motion.button>
                <motion.button
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2 md:p-6" onClick={() => setIsModalOpen(false)}>
            ...
            <motion.div
              ...
              className="relative w-full max-w-[95vw] md:max-w-[1200px] h-[90vh] bg-theme-bg border-4 md:border-8 border-theme-border shadow-[4px_4px_0px_0px_var(--color-theme-shadow)] flex flex-col"
              onClick={e => e.stopPropagation()}
            >
              {/* Modal Header */}
              <div className="flex items-center justify-between px-6 py-5 border-b-4 md:border-b-8 border-theme-border bg-[var(--color-primary)] flex-shrink-0">
                <div className="flex items-center gap-4">
                  <Target size={32} weight="bold" />
                  <div className="flex flex-col">
                    <span className="text-xs font-label font-black uppercase tracking-widest text-theme-text/70">MASS CONFIGURATION PROTOCOL</span>
                    <span className="text-2xl md:text-4xl font-display font-black uppercase tracking-tighter text-theme-text leading-none pt-1">
                ...
                <motion.button 
                  ...
                  className="text-theme-text bg-theme-bg border-4 border-theme-border p-2 hover:bg-red-500 hover:text-white transition-colors"
                >
                  <X size={24} weight="bold" />
                </motion.button>
              </div>
              
              <form onSubmit={handleBulkSubmit} className="flex flex-col flex-1 min-h-0 bg-theme-bg">
                <div className="overflow-auto flex-1 bg-theme-bg" data-lenis-prevent>
                  <table className="w-full text-left border-collapse min-w-[800px]">
                    <thead className="bg-[#f4f4f4] border-b-8 border-theme-border sticky top-0 z-10">
                      <tr>
                        <th className="px-6 py-4 font-display text-sm font-black text-theme-text uppercase tracking-widest border-r-4 border-theme-border">OPERATIVE (AGENT)</th>
                        <th className="px-6 py-4 font-display text-sm font-black text-theme-text uppercase tracking-widest border-r-4 border-theme-border w-48">TIER_CLASSIFICATION</th>
                        <th className="px-6 py-4 font-display text-sm font-black text-theme-text uppercase tracking-widest border-r-4 border-theme-border w-48">TRAJECTORY</th>
                        <th className="px-6 py-4 font-display text-sm font-black text-theme-text uppercase tracking-widest">ADDITIONAL_DATA</th>
                      </tr>
                    </thead>
                    <tbody>
                      {bulkRatings.map((rating, idx) => (
                        <tr key={rating.agent} className="border-b-4 border-theme-border hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 border-r-4 border-theme-border bg-gray-50">
                            <div className="flex items-center gap-4">
                              {rating.icon_url ? (
                                <img src={rating.icon_url} alt={rating.agent} className="w-12 h-12 border-4 border-theme-border bg-theme-bg" />
                              ) : (
                                <div className="w-12 h-12 bg-gray-200 border-4 border-theme-border" />
                              )}
                              <div className="flex flex-col">
                          ...
                          <td className="p-4 border-r-4 border-theme-border">
                            <div className="relative">
                              <select 
                                value={rating.tier} 
                                onChange={e => handleBulkChange(idx, 'tier', e.target.value)}
                                className={`w-full px-4 py-3 font-display text-xl font-black uppercase tracking-wider appearance-none border-4 border-theme-border cursor-pointer focus:outline-none focus:border-[var(--color-primary)] focus:shadow-[4px_4px_0px_0px_var(--color-theme-shadow)] transition-all
                                  ${rating.tier === 'S' ? 'bg-[#ff7e67] text-white' : 
                                    rating.tier === 'A' ? 'bg-[#ffb167] text-theme-text' : 
                                    rating.tier === 'B' ? 'bg-[#ffe467] text-theme-text' : 
                                    rating.tier === 'C' ? 'bg-[#b7ff67] text-theme-text' : 
                                    'bg-[#67ffd5] text-theme-text'}`}
                              >
                          ...
                          <td className="p-4 border-r-4 border-theme-border">
                            <div className="relative">
                              <select 
                                ...
                                className="w-full px-4 py-3 font-label text-sm font-black uppercase tracking-widest appearance-none border-4 border-theme-border bg-theme-bg cursor-pointer focus:outline-none focus:border-[var(--color-primary)] focus:shadow-[4px_4px_0px_0px_var(--color-theme-shadow)] transition-all"
                              >
                          ...
                          <td className="p-4">
                            <input 
                              ...
                              className="w-full px-4 py-3 font-label text-sm font-bold tracking-wider border-4 border-theme-border bg-theme-bg focus:outline-none focus:border-[var(--color-primary)] focus:shadow-[4px_4px_0px_0px_var(--color-theme-shadow)] transition-all"
                ...
                <div className="px-6 py-6 border-t-8 border-theme-border bg-[#f4f4f4] flex-shrink-0 flex flex-col sm:flex-row justify-end items-center gap-4">
                  <motion.button
                    whileHover={{ scale: 1.05, boxShadow: "4px 4px 0px 0px #111111" }}
                    whileTap={{ scale: 0.95, boxShadow: "0px 0px 0px 0px #111111" }}
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="w-full sm:w-auto px-6 py-4 border-4 border-theme-border bg-theme-bg font-display text-lg font-black uppercase tracking-tight text-theme-text transition-all"
                  >
                    ABORT SEQUENCE
                  </motion.button>
                  <motion.button
                    ...
                    type="submit"
                    disabled={loading}
                    className="w-full sm:w-auto bg-[var(--color-primary)] border-4 border-theme-border text-black px-8 py-4 font-display text-lg font-black uppercase tracking-tight transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
