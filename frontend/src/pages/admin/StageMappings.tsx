    ...
    <div className="font-['Inter']">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b border-black pb-6">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-md bg-[var(--color-background)] border-2 border-black p-0 shadow-[8px_8px_0px_0px_#111111] rounded-none overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0">
            <div className="px-5 py-4 border-b-2 border-black bg-white">
              <DialogHeader>
                <DialogTitle className="font-['Archivo_Black'] uppercase text-xl text-black">Create Mapping</DialogTitle>
              </DialogHeader>
            </div>
            <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 gap-5 bg-white">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-3 py-2 text-sm font-['JetBrains_Mono'] border-2 border-black bg-gray-50 focus:bg-white focus:outline-none focus:ring-4 focus:ring-yellow-200 transition-all rounded-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-3 py-2 text-sm font-['JetBrains_Mono'] font-bold border-2 border-black bg-gray-50 focus:bg-white focus:outline-none focus:ring-4 focus:ring-yellow-200 transition-all rounded-none"
                    required
                  />
                </div>
              </div>
              <div className="pt-2 mt-2">
                <button 
                  type="submit" 
                  disabled={loading}
                  className="w-full bg-black border-2 border-black px-4 py-3 text-[14px] font-['Archivo_Black'] uppercase text-white hover:bg-transparent hover:text-black transition-colors active:translate-y-1 shadow-[4px_4px_0px_0px_#111111] hover:shadow-none disabled:opacity-50 rounded-none"
                >
      ...
      <div className="mb-10">
        <h3 className="font-['JetBrains_Mono'] text-[11px] font-bold uppercase tracking-widest text-black bg-yellow-300 inline-block px-2 py-1 mb-2 border border-black">
          stage_mappings.db
        </h3>
              ...
              {initialFetch ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <TableRow key={i} className="border-b border-black last:border-b-0">
                    <TableCell className="p-3 border-r border-black last:border-r-0"><Skeleton className="h-4 w-48 bg-gray-200 rounded-none" /></TableCell>
                    <TableCell className="p-3 border-r border-black last:border-r-0"><Skeleton className="h-5 w-24 bg-gray-200 rounded-none" /></TableCell>
                ...
                table.getRowModel().rows.map(row => (
                  <TableRow key={row.id} className="border-b border-black last:border-b-0 hover:bg-yellow-50 transition-none">
                    {row.getVisibleCells().map(cell => (
                      <TableCell key={cell.id} className="px-4 py-3 border-r border-black last:border-r-0">
