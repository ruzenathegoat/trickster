        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <span className="font-numeric font-black bg-black text-[var(--color-primary)] px-2 py-0.5 text-sm border-2 border-black shadow-[2px_2px_0px_#000]" title="VLR Average Rating">
          ...
          <div className="flex gap-2 mt-2 flex-wrap">
            <span className="font-label text-[9px] font-black uppercase tracking-widest border-2 border-black px-1.5 py-0.5 bg-gray-100 shadow-[1px_1px_0px_#000]">
              {player.current_role}
            </span>
            {player.is_igl && (
              <span className="flex items-center gap-1 font-label text-[9px] font-black uppercase tracking-widest border-2 border-black px-1.5 py-0.5 bg-[var(--color-primary)] text-black shadow-[1px_1px_0px_#000]">
                <Star weight="fill" size={10} /> IGL
              </span>
            )}
            {player.playstyle_tags?.slice(0, 2).map((tag, i) => (
              <span key={i} className="font-label text-[9px] font-bold uppercase tracking-widest border-2 border-black px-1.5 py-0.5 bg-white shadow-[1px_1px_0px_#000]">
                {tag}
              </span>
            ))}
          </div>
      ...
      {reason && (
        <div className="mt-4 pt-3 border-t-4 border-black">
          <div className="flex items-center justify-between mb-2">
            <div className="inline-block bg-[#10b981] text-black font-label text-[10px] font-black uppercase tracking-widest px-2 py-0.5 border-2 border-black shadow-[2px_2px_0px_#000]">
              AI Synergy Match
            </div>
            {synergyScore !== undefined && (
              <div className="font-numeric font-black text-xs px-2 border-2 border-black bg-black text-[#10b981] shadow-[2px_2px_0px_#000]">
                {synergyScore} PTS
              </div>
            )}
          </div>
