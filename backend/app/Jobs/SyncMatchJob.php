...
class SyncMatchJob implements ShouldQueue
{
    ...
    public function handle(StageMappingResolver $stageResolver): void
    {
            ...
            if (count($teams) >= 2) {
                ...
                if ($scoreSpans->count() >= 3) {
                    if (str_contains($scoreSpans->eq(0)->attr('class') ?? '', 'winner')) {
                        $winnerTeamId = $teamA->id;
                    } elseif (str_contains($scoreSpans->eq(2)->attr('class') ?? '', 'winner')) {
                        $winnerTeamId = $teamB->id;
                    }
                }

                $event = Event::with('stageFormatProfile')
                    ->where('vlr_event_id', $this->queueItem->vlr_event_id)
                    ->first();
                $resolvedStage = $event?->stageFormatProfile === null
                    ? null
                    : $stageResolver->resolve($event->stageFormatProfile->key, $stageLabel);
                ...
                MatchData::updateOrCreate(
                    ...
                    [
                        ...
                        'stage_label_id' => $resolvedStage?->mappingId,
                        'stage_resolution_source' => $resolvedStage?->source,
                        'stage_resolved_at' => $resolvedStage === null ? null : now(),
                        'best_of' => $format,
                    ]
                );
